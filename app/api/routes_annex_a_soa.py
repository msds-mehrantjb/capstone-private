...
# =========================================================
def _save_annex_generation_checkpoint(
    year: int,
    controls: list[dict],
    generation_metadata: dict,
) -> None:
    """Atomically persist rows whose final LLM justification is complete."""
    checkpoint_doc = {
        "controls": controls,
        "generation_metadata": generation_metadata,
    }
    _set_annex_doc_state(
        checkpoint_doc,
        status="In Progress",
        submitted=False,
        read_only=False,
    ...
    with open(temporary_path, "w", encoding="utf-8") as checkpoint_file:
        json.dump(checkpoint_doc, checkpoint_file, indent=2)
        checkpoint_file.flush()
        os.fsync(checkpoint_file.fileno())
    os.replace(temporary_path, path)


def _build_annex_from_risk_eval(year: int, resume_doc: dict | None = None) -> dict:
    ...
    generation_metadata = {
        ...
        "fallback_used": False,
        "fallback_details": [],
        "checkpoint_status": "generating",
        "checkpoint_completed_control_ids": [],
        "checkpoint_errors": [],
    }
    ...
    if isinstance(resume_doc, dict):
        resume_metadata = resume_doc.get("generation_metadata", {})
        if (
            isinstance(resume_metadata, dict)
            and resume_metadata.get("checkpoint_status") in {"generating", "failed"}
        ):
            resumed_controls = {
                _normalize_text(item.get("control_id") or item.get("control")): item
                for item in _all_controls(resume_doc)
                if _normalize_text(item.get("justification"))
                and item.get("justification_source") == "llm_reasoning"
            }

    justification_errors = []
    completed_controls = []
    generation_metadata["checkpoint_total_controls"] = len(controls)
    for control in controls:
        control_id = _normalize_text(control.get("control_id") or control.get("control"))
        try:
            resumed_control = resumed_controls.get(control_id)
            if resumed_control is not None:
                control["justification"] = resumed_control["justification"]
                generation_note = "Final SoA justification restored from generation checkpoint."
            else:
                control["justification"] = _generate_annex_row_justification_with_llm(year, control)
                generation_note = "Final SoA justification generated by dedicated LLM reasoning prompt."
            control["justification_source"] = "llm_reasoning"
            control["generation_note"] = generation_note
            ...
            generation_metadata["checkpoint_completed_control_ids"] = [
                item.get("control_id") for item in completed_controls
            ]
            generation_metadata["checkpoint_completed_controls"] = len(completed_controls)
            _save_annex_generation_checkpoint(year, completed_controls, generation_metadata)
        except Exception as e:
    ...
    if justification_errors:
        ...
        generation_metadata["checkpoint_status"] = "failed"
        generation_metadata["checkpoint_errors"] = justification_errors
        _save_annex_generation_checkpoint(year, completed_controls, generation_metadata)
        raise ValueError(
            "LLM reasoning justification failed for one or more Annex A & SoA controls. "
            f"Checkpoint saved {len(completed_controls)} of {len(controls)} successful rows; "
            "run /create again to resume the remaining rows. "
            "No fallback justifications were saved. "
            f"Failures: {'; '.join(justification_errors)}"
        )

    generation_metadata["checkpoint_status"] = "complete"
    generation_metadata["checkpoint_errors"] = []
...
def create_annex_a_soa(payload: CreateRequest):
    ...
    current_generation_metadata = current.get("generation_metadata", {})
    has_resumable_checkpoint = (
        isinstance(current_generation_metadata, dict)
        and current_generation_metadata.get("checkpoint_status") in {"generating", "failed"}
    )

    if len(_all_controls(current)) > 0 and not payload.force and not has_resumable_checkpoint:
        return {
            "success": False,
    ...
    try:
        new_doc = _build_annex_from_risk_eval(
            year,
            resume_doc=current if has_resumable_checkpoint and not payload.force else None,
        )
        ...
        checkpoint = _load_annex_doc_or_blank(year)
        return {
            "success": False,
